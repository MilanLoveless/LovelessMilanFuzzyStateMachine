...
{
	// Run the SubMachine
	if(this->subMachine != NULL) this->subMachine->RunOneStep();
	...
	// Normalize link recommendations
...
void FuzzyStateMachine::AddNode(char *name, FuzzyStateMachine *subMachine)
{
	...
	if(this->pCurrentNode == NULL) this->pCurrentNode = pNode;
}
...
NODE* FuzzyStateMachine::FindNode(char *name)
{
	...
	{
		NODE* pNode = _Nodes[n];
		if(strcmp(pNode->name, name) == 0) return pNode;
	}
	return NULL;
}
...
{
	if(this->pCurrentNode != NULL) return this->pCurrentNode->name;
}

void FuzzyStateMachine::RunOneStep()
{
	if(this->pCurrentNode != NULL) this->pCurrentNode->RunOneStep();
}
